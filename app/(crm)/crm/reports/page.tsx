...
export default function ReportsPage() {
  ...
  if (loading) return <div style={{ padding: 40, color: '#8E8E93' }}>Loading report data...</div>

  return (
    <div style={{ padding: '36px 44px', maxWidth: 1400 }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 32, fontWeight: 300, color: '#1C1C1E', margin: 0 }}>Report Builder</h1>
        <p style={{ fontSize: 14, color: '#8E8E93', margin: '4px 0 0' }}>Build custom reports from your CRM data</p>
      </div>

      {/* Controls */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
        {/* Metrics */}
        <div style={{ background: 'white', borderRadius: 16, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', padding: 20 }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: '#8E8E93', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 12 }}>Select Metrics</div>
        ...
        {/* Dimension + Filters */}
        <div style={{ background: 'white', borderRadius: 16, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', padding: 20 }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: '#8E8E93', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 12 }}>Group By & Filters</div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <select value={selectedDimension} onChange={e => setSelectedDimension(e.target.value)} style={{ padding: '6px 12px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', fontSize: 12, background: 'white' }}>
              {dimensions.map(d => <option key={d.id} value={d.id}>{d.label}</option>)}
            </select>
            <select value={filterSpecialty} onChange={e => setFilterSpecialty(e.target.value)} style={{ padding: '6px 12px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', fontSize: 12, background: 'white' }}>
            ...
            <select value={filterSource} onChange={e => setFilterSource(e.target.value)} style={{ padding: '6px 12px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', fontSize: 12, background: 'white' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${activeMetrics.length}, 1fr)`, gap: 12, marginBottom: 24 }}>
        {activeMetrics.map(m => {
          ...
          return (
            <div key={m.id} style={{ background: 'white', borderRadius: 16, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', padding: '20px', textAlign: 'center' }}>
              <div style={{ fontSize: 32, fontWeight: 300, color: '#00B5D6', lineHeight: 1 }}>{display}</div>
              <div style={{ fontSize: 11, color: '#8E8E93', marginTop: 6, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{m.label}</div>
            </div>
          )
        })}
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ borderBottom: '0.5px solid rgba(0,0,0,0.06)', background: 'rgba(0,0,0,0.02)' }}>
              <th style={{ textAlign: 'left', padding: '12px 16px', fontWeight: 500, color: '#8E8E93', fontSize: 11, textTransform: 'uppercase' }}>{dim.label.replace('By ', '')}</th>
              {activeMetrics.map(m => (
                <th key={m.id} style={{ textAlign: 'right', padding: '12px 16px', fontWeight: 500, color: '#8E8E93', fontSize: 11, textTransform: 'uppercase' }}>{m.label}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {Object.entries(groups).sort((a, b) => b[1].length - a[1].length).map(([group, groupLeads]) => (
              <tr key={group} style={{ borderBottom: '0.5px solid rgba(0,0,0,0.04)' }}>
                <td style={{ padding: '12px 16px', fontWeight: 500, color: '#1C1C1E', textTransform: 'capitalize' }}>{group}</td>
                {activeMetrics.map(m => {
                  const val = m.calc(groupLeads)
                  const display = m.id.includes('value') || m.id.includes('revenue') ? `$${Math.round(val / 1000)}K` : m.id.includes('rate') ? `${val}%` : val.toString()
                  return <td key={m.id} style={{ textAlign: 'right', padding: '12px 16px', color: '#1C1C1E' }}>{display}</td>
                })}
              </tr>
            ))}
            {/* Total row */}
            <tr style={{ borderTop: '2px solid #E6E6E6', background: 'rgba(0,0,0,0.02)' }}>
              <td style={{ padding: '12px 16px', fontWeight: 600, color: '#1C1C1E' }}>Total</td>
              {activeMetrics.map(m => {
                const val = m.calc(filtered)
                const display = m.id.includes('value') || m.id.includes('revenue') ? `$${Math.round(val / 1000)}K` : m.id.includes('rate') ? `${val}%` : val.toString()
      ...
      {/* Visual bars */}
      <div style={{ background: 'white', borderRadius: 16, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', padding: 24, marginTop: 20 }}>
        <h3 style={{ fontSize: 14, fontWeight: 600, margin: '0 0 16px', color: '#8E8E93', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Distribution</h3>
        {Object.entries(groups).sort((a, b) => b[1].length - a[1].length).map(([group, groupLeads]) => {
          ...
          return (
            <div key={group} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
              <div style={{ width: 120, fontSize: 12, color: '#8E8E93', textAlign: 'right', textTransform: 'capitalize' }}>{group}</div>
              ...
              {pct <= 12 && <span style={{ fontSize: 12, fontWeight: 600, color: '#8E8E93', minWidth: 24 }}>{groupLeads.length}</span>}
              <span style={{ fontSize: 11, color: '#C7C7CC', minWidth: 32 }}>{pct}%</span>
            </div>
          )
        })}
      </div>
