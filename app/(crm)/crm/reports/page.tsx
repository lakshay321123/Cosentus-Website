...
export default function ReportsPage() {
  ...
  if (loading) return <div style={{ padding: 40, color: '#9ca3af' }}>Loading report data...</div>

  return (
    <div style={{ padding: '36px 44px', maxWidth: 1400 }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 32, fontWeight: 300, color: '#1f2937', margin: 0 }}>Report Builder</h1>
        <p style={{ fontSize: 14, color: '#9ca3af', margin: '4px 0 0' }}>Build custom reports from your CRM data</p>
      </div>
            ...
            {Object.entries(groups).sort((a, b) => b[1].length - a[1].length).map(([group, groupLeads]) => (
              <tr key={group} style={{ borderBottom: '0.5px solid #e5e7eb' }}>
                ...
                {activeMetrics.map(m => {
                  ...
                  const display = m.id.includes('value') || m.id.includes('revenue') ? `$${Math.round(val / 1000)}K` : m.id.includes('rate') ? `${val}%` : val.toString()
                  return <td key={m.id} style={{ textAlign: 'right', padding: '12px 16px', color: '#1f2937' }}>{display}</td>
                })}
              </tr>
            ))}
            {/* Total row */}
            <tr style={{ borderTop: '2px solid #E6E6E6', background: '#fafbfc' }}>
              <td style={{ padding: '12px 16px', fontWeight: 600, color: '#1f2937' }}>Total</td>
              {activeMetrics.map(m => {
                const val = m.calc(filtered)
                const display = m.id.includes('value') || m.id.includes('revenue') ? `$${Math.round(val / 1000)}K` : m.id.includes('rate') ? `${val}%` : val.toString()
        ...
        {Object.entries(groups).sort((a, b) => b[1].length - a[1].length).map(([group, groupLeads]) => {
          ...
          return (
            <div key={group} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
              <div style={{ width: 120, fontSize: 12, color: '#9ca3af', textAlign: 'right', textTransform: 'capitalize' }}>{group}</div>
              ...
              {pct <= 12 && <span style={{ fontSize: 12, fontWeight: 600, color: '#9ca3af', minWidth: 24 }}>{groupLeads.length}</span>}
              <span style={{ fontSize: 11, color: '#d1d5db', minWidth: 32 }}>{pct}%</span>
            </div>
          )
        })}
      </div>
