      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
        ...
        <div style={{ background: 'white', borderRadius: 16, border: 'none', boxShadow: '0 1px 3px #E6E6E6, 0 4px 12px #D6EBF2', padding: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            <select value={filterSpecialty} onChange={e => setFilterSpecialty(e.target.value)} style={{ padding: '6px 12px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px #E6E6E6, 0 4px 12px #D6EBF2', fontSize: 12, background: 'white' }}>
            ...
            <select value={filterSource} onChange={e => setFilterSource(e.target.value)} style={{ padding: '6px 12px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px #E6E6E6, 0 4px 12px #D6EBF2', fontSize: 12, background: 'white' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          ...
          <tbody>
            {Object.entries(groups).sort((a, b) => b[1].length - a[1].length).map(([group, groupLeads]) => (
              <tr key={group} style={{ borderBottom: '0.5px solid #E6E6E6' }}>
                <td style={{ padding: '12px 16px', fontWeight: 500, color: '#000000', textTransform: 'capitalize' }}>{group}</td>
                {activeMetrics.map(m => {
                  const val = m.calc(groupLeads)
                  const display = m.id.includes('value') || m.id.includes('revenue') ? `$${Math.round(val / 1000)}K` : m.id.includes('rate') ? `${val}%` : val.toString()
                  return <td key={m.id} style={{ textAlign: 'right', padding: '12px 16px', color: '#000000' }}>{display}</td>
                })}
              </tr>
            ))}
            {/* Total row */}
