...
export default function CampaignsPage() {
  ...
  if (loading) return <div style={{ padding: 40, color: '#9ca3af' }}>Loading campaigns...</div>

  return (
    <div style={{ padding: '36px 44px', maxWidth: 1400 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 32, fontWeight: 300, color: '#1f2937', margin: 0 }}>Campaigns</h1>
          <p style={{ fontSize: 14, color: '#9ca3af', margin: '4px 0 0' }}>Track marketing campaigns and measure ROI</p>
        </div>
          ...
          return (
            <div key={c.id} style={{ background: 'white', borderRadius: 16, border: 'none', boxShadow: '0 1px 3px #e5e7eb, 0 4px 12px #f3f4f6', padding: '20px 24px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    <span style={{ fontSize: 16, fontWeight: 600, color: '#1f2937' }}>{c.name}</span>
                    ...
                    <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 4, background: '#f3f4f6', color: '#9ca3af' }}>{typeLabels[c.type] || c.type}</span>
                  </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 16, paddingTop: 12, borderTop: '1px solid #F5F5F5', fontSize: 12 }}>
                <div><span style={{ color: '#9ca3af' }}>Budget</span><div style={{ fontWeight: 600, color: '#1f2937', marginTop: 2 }}>${(c.budget || 0).toLocaleString()}</div></div>
                <div><span style={{ color: '#9ca3af' }}>Spent</span><div style={{ fontWeight: 600, color: '#1f2937', marginTop: 2 }}>${(c.spent || 0).toLocaleString()}</div></div>
                <div><span style={{ color: '#9ca3af' }}>Leads</span><div style={{ fontWeight: 600, color: '#1f2937', marginTop: 2 }}>{c.leads_generated}</div></div>
                <div><span style={{ color: '#9ca3af' }}>Revenue</span><div style={{ fontWeight: 600, color: '#1f2937', marginTop: 2 }}>${Math.round((c.revenue_attributed || 0) / 1000)}K</div></div>
                <div><span style={{ color: '#9ca3af' }}>ROI</span><div style={{ fontWeight: 600, color: roi > 0 ? '#085041' : '#791F1F', marginTop: 2 }}>{roi > 0 ? '+' : ''}{roi}%</div></div>
              </div>
            </div>
          )
        })}
