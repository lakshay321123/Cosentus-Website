...
export default function EmailsPage() {
  ...
  const toHtml = () => {
    ...
    for (const b of blocks) {
      ...
      else if (b.type === 'text') { const txt = `<div style="${pad};${bg};${align};font-size:${b.fontSize||15}px;line-height:1.7;color:${b.color||'#333'};font-family:${b.fontFamily||'Arial'},sans-serif;${b.fontWeight === 'bold' ? 'font-weight:bold;' : ''}${b.fontStyle === 'italic' ? 'font-style:italic;' : ''}">${fillVars(b.content)}</div>`; const safeUrl = (b.linkUrl || '').startsWith('http') ? b.linkUrl : ''; html += safeUrl ? `<a href="${safeUrl}" style="text-decoration:none;color:inherit">${txt}</a>` : txt }
      else if (b.type === 'heading') { const txt = `<div style="${pad};${bg};${align}"><h2 style="margin:0;font-size:${b.fontSize||24}px;color:${b.color||'#000'};font-weight:${b.fontWeight||'bold'};font-family:${b.fontFamily||'Arial'},sans-serif;${b.fontStyle === 'italic' ? 'font-style:italic;' : ''}">${fillVars(b.content)}</h2></div>`; const safeUrl = (b.linkUrl || '').startsWith('http') ? b.linkUrl : ''; html += safeUrl ? `<a href="${safeUrl}" style="text-decoration:none;color:inherit">${txt}</a>` : txt }
      ...
      else if (b.type === 'image_text') html += `<div style="${pad};${bg};display:flex;gap:16px;align-items:center"><div style="width:${b.imgWidth||40}%;flex-shrink:0"><img src="${b.imgUrl||''}" alt="${b.imgAlt||''}" style="width:100%;border-radius:${b.borderRadius||0}px"></div><div style="flex:1;font-size:${b.fontSize||14}px;color:${b.color||'#333'};font-family:${b.fontFamily||'Arial'},sans-serif;${b.fontWeight === 'bold' ? 'font-weight:bold;' : ''}${b.fontStyle === 'italic' ? 'font-style:italic;' : ''}">${fillVars(b.content)}</div></div>`
    }
