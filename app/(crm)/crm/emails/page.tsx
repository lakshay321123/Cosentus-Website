      ...
      {tab === 'saved' ? (
        ...
        <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
          ...
          <div style={{ width: 180, borderRight: '1px solid #E6E6E6', overflowY: 'auto', padding: 12, flexShrink: 0 }}>
            ...
            <div style={{ fontSize: 11, fontWeight: 600, color: '#CCCCCC', letterSpacing: '0.08em', marginTop: 16, marginBottom: 8 }}>TEMPLATES</div>
            {Object.keys(premadeTemplates).map(name => (
              <button key={name} onClick={() => { pushHistory(); setBlocks(premadeTemplates[name].map(b => ({ ...b, id: `t-${Date.now()}-${Math.random().toString(36).slice(2,6)}` }))); setSelected(null) }} style={{ width: '100%', fontSize: 11, padding: '6px 10px', borderRadius: 6, border: '1px solid #00B5D6', background: '#D6EBF2', cursor: 'pointer', textAlign: 'left', marginBottom: 3, color: '#00B5D6', fontWeight: 500, ...S }}>
                {name}
              </button>
            ))}
          </div>
            ...
            <div style={{ maxWidth: mobilePreview ? 360 : 600, margin: '0 auto', background: '#fff', borderRadius: 8, boxShadow: '0 2px 12px rgba(0,0,0,0.08)', overflow: 'hidden', transition: 'max-width 0.3s' }}>
              {blocks.map((b, idx) => (
                <div key={b.id} onClick={() => setSelected(b.id)} style={{ position: 'relative', outline: selected === b.id ? '2px solid #00B5D6' : '1px solid transparent', cursor: 'pointer', transition: 'outline 0.15s' }}>
                  ...
                  {b.type === 'text' && <div style={{ padding: `${b.padding}px 32px`, ...bgStyle(b), textAlign: b.align as any, fontSize: b.fontSize, color: b.color, fontWeight: b.fontWeight as any, fontStyle: b.fontStyle as any, fontFamily: b.fontFamily || 'Arial', lineHeight: b.lineHeight ? b.lineHeight/100 : 1.7, letterSpacing: b.letterSpacing || 0 }} contentEditable suppressContentEditableWarning onBlur={e => updateBlock(b.id, { content: e.currentTarget.innerText })}>{fillVars(b.content)}</div>}
