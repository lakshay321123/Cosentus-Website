...
export default function TasksPage() {
  ...
  if (loading) return <div style={{ padding: 40, color: '#8E8E93' }}>Loading tasks...</div>

  return (
    <div style={{ padding: '36px 44px', maxWidth: 1200 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 32, fontWeight: 300, color: '#1C1C1E', margin: 0 }}>Tasks</h1>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <div style={{ display: 'flex', borderRadius: 10, overflow: 'hidden', border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)' }}>
      ...
      {showAdd && (
        <form onSubmit={handleAdd} style={{ background: 'white', borderRadius: 16, border: '1px solid rgba(0,181,214,0.3)', padding: 24, marginBottom: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <input name="title" placeholder="Task title *" required style={{ padding: '10px 14px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', fontSize: 13, gridColumn: '1 / -1' }} />
            <input name="description" placeholder="Description (optional)" style={{ padding: '10px 14px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', fontSize: 13, gridColumn: '1 / -1' }} />
            ...
            <input name="due_date" type="datetime-local" style={{ padding: '10px 14px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', fontSize: 13 }} />
            <select name="priority" style={{ padding: '10px 14px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', fontSize: 13, background: 'white' }}>
              <option value="high">High Priority</option>
              <option value="medium" selected>Medium Priority</option>
              <option value="low">Low Priority</option>
            </select>
            <input name="assigned_to" placeholder="Assigned to" style={{ padding: '10px 14px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', fontSize: 13 }} />
          </div>
      ...
      {filtered.length === 0 ? (
        <div style={{ background: 'white', borderRadius: 16, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', padding: '60px 40px', textAlign: 'center' }}>
          <div style={{ fontSize: 14, color: '#8E8E93' }}>No {filter === 'all' ? '' : filter} tasks</div>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {filtered.map(task => (
            <div key={task.id} style={{
              ...
              {/* Info */}
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, fontWeight: 500, color: '#1C1C1E', textDecoration: task.status === 'completed' ? 'line-through' : 'none' }}>{task.title}</div>
                <div style={{ display: 'flex', gap: 8, marginTop: 6, flexWrap: 'wrap', alignItems: 'center' }}>
                  ...
                  {task.assigned_to && <span style={{ fontSize: 11, color: '#C7C7CC' }}>{task.assigned_to}</span>}
                </div>
              </div>

              <PriorityBadge p={task.priority} />

              <button onClick={() => deleteTask(task.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#C7C7CC', fontSize: 16, padding: '4px 8px' }} title="Delete">×</button>
            </div>
          ))}
        </div>
      )}
