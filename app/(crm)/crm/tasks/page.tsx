...
export default function TasksPage() {
  ...
  if (loading) return <div style={{ padding: 40, color: '#616161' }}>Loading tasks...</div>

  return (
    <div style={{ padding: '32px 40px', maxWidth: 1200 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 300, color: '#000', margin: 0 }}>Tasks</h1>
          <p style={{ fontSize: 14, color: '#616161', margin: '4px 0 0' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={() => setShowAdd(!showAdd)} style={{ background: '#00B5D6', color: 'white', border: 'none', borderRadius: 8, padding: '10px 24px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>+ Add Task</button>
        </div>
      </div>
      ...
      {showAdd && (
        <form onSubmit={handleAdd} style={{ background: 'white', borderRadius: 12, border: '1px solid #00B5D6', padding: 24, marginBottom: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <input name="title" placeholder="Task title *" required style={{ padding: '10px 14px', borderRadius: 6, border: '1px solid #E6E6E6', fontSize: 13, gridColumn: '1 / -1' }} />
            <input name="description" placeholder="Description (optional)" style={{ padding: '10px 14px', borderRadius: 6, border: '1px solid #E6E6E6', fontSize: 13, gridColumn: '1 / -1' }} />
            ...
            <input name="due_date" type="datetime-local" style={{ padding: '10px 14px', borderRadius: 6, border: '1px solid #E6E6E6', fontSize: 13 }} />
            <select name="priority" style={{ padding: '10px 14px', borderRadius: 6, border: '1px solid #E6E6E6', fontSize: 13, background: 'white' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
            <button type="submit" style={{ background: '#00B5D6', color: 'white', border: 'none', borderRadius: 6, padding: '8px 20px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Create Task</button>
      ...
      {filtered.length === 0 ? (
        <div style={{ background: 'white', borderRadius: 12, border: '1px solid #E6E6E6', padding: '60px 40px', textAlign: 'center' }}>
          <div style={{ fontSize: 14, color: '#616161' }}>No {filter === 'all' ? '' : filter} tasks</div>
        </div>
      ) : (
          ...
          {filtered.map(task => (
            <div key={task.id} style={{
              ...
              <button onClick={() => toggleComplete(task)} style={{
                width: 22, height: 22, borderRadius: 6, border: `2px solid ${task.status === 'completed' ? '#00B5D6' : '#CCCCCC'}`,
              ...
              {/* Info */}
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, fontWeight: 500, color: '#000', textDecoration: task.status === 'completed' ? 'line-through' : 'none' }}>{task.title}</div>
                <div style={{ display: 'flex', gap: 8, marginTop: 6, flexWrap: 'wrap', alignItems: 'center' }}>
                  ...
                  {task.assigned_to && <span style={{ fontSize: 11, color: '#CCCCCC' }}>{task.assigned_to}</span>}
                </div>
              </div>

              <PriorityBadge p={task.priority} />

              <button onClick={() => deleteTask(task.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#CCCCCC', fontSize: 16, padding: '4px 8px' }} title="Delete">×</button>
            </div>
          ))}
        </div>
      )}
