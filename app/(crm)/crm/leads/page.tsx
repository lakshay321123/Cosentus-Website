    ...
    <div style={{ padding: '36px 44px', maxWidth: '100%', boxSizing: 'border-box' as const }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ fontSize: 14, color: '#000000', margin: '4px 0 0' }}>{filtered.length} of {leads.length} leads</p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <label style={{ background: 'white', color: '#000000', border: 'none', boxShadow: '0 1px 3px #E6E6E6, 0 4px 12px #D6EBF2', borderRadius: 12, padding: '10px 20px', fontSize: 13, fontWeight: 500, display: 'inline-flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
          ...
          <a href="/api/crm/export" style={{ background: 'white', color: '#000000', border: 'none', boxShadow: '0 1px 3px #E6E6E6, 0 4px 12px #D6EBF2', borderRadius: 12, padding: '10px 20px', fontSize: 13, fontWeight: 500, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
      ...
      {showAdd && (
        <div style={{ background: 'white', borderRadius: 16, border: '1px solid rgba(0,181,214,0.3)', padding: 24, marginBottom: 20 }}>
          ...
          <form id="add-lead-form" onSubmit={handleAddLead} style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
            ...
            <select name="specialty" style={{ padding: '10px 14px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px #E6E6E6, 0 4px 12px #D6EBF2', fontSize: 13, color: '#000000', background: 'white' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
            ...
            <button onClick={() => setShowAdd(false)} style={{ background: 'transparent', color: '#000000', border: 'none', boxShadow: '0 1px 3px #E6E6E6, 0 4px 12px #D6EBF2', borderRadius: 10, padding: '8px 20px', fontSize: 13, cursor: 'pointer' }}>Cancel</button>
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        <select value={filterSpec} onChange={e => setFilterSpec(e.target.value)} style={{ padding: '8px 12px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px #E6E6E6, 0 4px 12px #D6EBF2', fontSize: 13, color: '#000000', background: 'white' }}>
        ...
        <select value={filterTemp} onChange={e => setFilterTemp(e.target.value)} style={{ padding: '8px 12px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px #E6E6E6, 0 4px 12px #D6EBF2', fontSize: 13, color: '#000000', background: 'white' }}>
          <option value="all">All Temps</option>
          <option value="hot">Hot</option>
          <option value="warm">Warm</option>
          <option value="cold">Cold</option>
        </select>
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} style={{ padding: '8px 12px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px #E6E6E6, 0 4px 12px #D6EBF2', fontSize: 13, color: '#000000', background: 'white' }}>
        ...
        <select value={sortBy} onChange={e => setSortBy(e.target.value as any)} style={{ padding: '8px 12px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px #E6E6E6, 0 4px 12px #D6EBF2', fontSize: 13, color: '#000000', background: 'white' }}>
          ...
          <thead>
            <tr style={{ borderBottom: '0.5px solid #E6E6E6', background: '#00B5D6', color: '#fff' }}>
              ...
              {['Contact', 'Specialty', 'AI Score', 'Temp', 'Stage', 'Source', 'Assigned', 'MRR'].map(h => (
                <th key={h} style={{ textAlign: h === 'MRR' ? 'right' : 'left', padding: '12px 16px', fontWeight: 500, color: '#000000', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {filtered.map(lead => (
              <tr key={lead.id} style={{ borderBottom: '0.5px solid #E6E6E6', cursor: 'pointer', transition: 'background 0.15s', background: selected.has(lead.id) ? 'rgba(0,181,214,0.04)' : 'transparent' }}
                ...
                <td style={{ padding: '14px 16px', color: '#000000' }}>{sourceLabels[lead.source] || lead.source}</td>
                <td style={{ padding: '14px 16px', color: '#000000', fontSize: 12 }}>{lead.assigned_to || '—'}</td>
