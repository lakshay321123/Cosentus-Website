...
export default function LeadDetailPage() {
  ...
  if (loading) return <div style={{ padding: 40, color: '#616161' }}>Loading lead...</div>
  if (!lead) return <div style={{ padding: 40, color: '#616161' }}>Lead not found</div>
  ...
  return (
    <div style={{ padding: '32px 40px', maxWidth: 1200 }}>
      {/* Back */}
      <Link href="/crm/leads" style={{ fontSize: 13, color: '#616161', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 4, marginBottom: 20 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 32 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 300, color: '#000', margin: 0 }}>{lead.first_name} {lead.last_name}</h1>
          <p style={{ fontSize: 16, color: '#616161', margin: '4px 0 0' }}>{lead.practice_name}</p>
          <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
            <TempBadge temp={lead.temperature} />
            <span style={{ fontSize: 12, padding: '4px 10px', borderRadius: 4, background: '#F5F5F5', color: '#616161', textTransform: 'capitalize' }}>{lead.specialty?.replace('_', ' ')}</span>
            <span style={{ fontSize: 12, padding: '4px 10px', borderRadius: 4, background: '#F5F5F5', color: '#616161' }}>{sourceLabels[lead.source] || lead.source}</span>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        ...
        <div>
          <div style={{ background: 'white', borderRadius: 12, border: '1px solid #E6E6E6', padding: 24, marginBottom: 20 }}>
            <h3 style={{ fontSize: 14, fontWeight: 600, margin: '0 0 16px', textTransform: 'uppercase', letterSpacing: '0.06em', color: '#616161' }}>Contact Details</h3>
            {[
              ...
              <div key={f.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #F5F5F5', fontSize: 13 }}>
                <span style={{ color: '#616161' }}>{f.label}</span>
                <span style={{ fontWeight: 500, color: '#000' }}>{f.value}</span>
              </div>
            ))}
          </div>
          ...
          {lead.notes && (
            <div style={{ background: 'white', borderRadius: 12, border: '1px solid #E6E6E6', padding: 24 }}>
              <h3 style={{ fontSize: 14, fontWeight: 600, margin: '0 0 12px', textTransform: 'uppercase', letterSpacing: '0.06em', color: '#616161' }}>Notes</h3>
              <p style={{ fontSize: 14, lineHeight: 1.6, color: '#000', margin: 0 }}>{lead.notes}</p>
            </div>
          )}
        </div>
        ...
        <div>
          <div style={{ background: 'white', borderRadius: 12, border: '1px solid #E6E6E6', padding: 24 }}>
            <h3 style={{ fontSize: 14, fontWeight: 600, margin: '0 0 16px', textTransform: 'uppercase', letterSpacing: '0.06em', color: '#616161' }}>Activity Timeline</h3>

            {/* Add note */}
            <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
              <input value={note} onChange={e => setNote(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddNote()}
                placeholder="Add a note..." style={{ flex: 1, padding: '8px 12px', borderRadius: 6, border: '1px solid #E6E6E6', fontSize: 13, outline: 'none' }} />
              <button onClick={handleAddNote} style={{ background: '#00B5D6', color: 'white', border: 'none', borderRadius: 6, padding: '8px 16px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Add</button>
            </div>
                ...
                {activities.map((a, i) => {
                  ...
                  return (
                    <div key={a.id} style={{ display: 'flex', gap: 12, padding: '12px 0', borderBottom: i < activities.length - 1 ? '1px solid #F5F5F5' : 'none' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 13, color: '#000' }}>{a.description}</div>
