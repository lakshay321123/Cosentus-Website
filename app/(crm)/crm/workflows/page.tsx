...
export default function WorkflowsPage() {
  ...
  if (loading) return <div style={{ padding: 40, color: '#8E8E93' }}>Loading workflows...</div>

  return (
    <div style={{ padding: '36px 44px', maxWidth: 1200 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 32, fontWeight: 300, color: '#1C1C1E', margin: 0 }}>Workflows</h1>
          <p style={{ fontSize: 14, color: '#8E8E93', margin: '4px 0 0' }}>Automate actions based on triggers</p>
        </div>
      ...
      {showCreate && (
        <form onSubmit={handleCreate} style={{ background: 'white', borderRadius: 16, border: '1px solid rgba(0,181,214,0.3)', padding: 24, marginBottom: 20 }}>
          ...
          <input name="name" placeholder="Workflow name *" required style={{ width: '100%', padding: '10px 14px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', fontSize: 13, marginBottom: 12, boxSizing: 'border-box' as const }} />
          <input name="description" placeholder="Description" style={{ width: '100%', padding: '10px 14px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', fontSize: 13, marginBottom: 16, boxSizing: 'border-box' as const }} />

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div style={{ background: 'rgba(0,0,0,0.02)', borderRadius: 12, padding: 16 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: '#00B5D6', marginBottom: 8 }}>WHEN (Trigger)</div>
              <select name="trigger_type" required style={{ width: '100%', padding: '8px 12px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', fontSize: 13, background: 'white', marginBottom: 8 }}>
                {Object.entries(triggerLabels).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
              </select>
              <div style={{ fontSize: 11, color: '#8E8E93', marginBottom: 8 }}>Condition (optional):</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 1fr', gap: 6 }}>
                <select name="condition_field" style={{ padding: '6px 8px', borderRadius: 4, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', fontSize: 12, background: 'white' }}>
                ...
                <select name="condition_op" style={{ padding: '6px 8px', borderRadius: 4, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', fontSize: 12, background: 'white' }}>
                ...
                <input name="condition_value" placeholder="Value" style={{ padding: '6px 8px', borderRadius: 4, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', fontSize: 12 }} />
              </div>
            </div>
            <div style={{ background: 'rgba(0,0,0,0.02)', borderRadius: 12, padding: 16 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: '#00B5D6', marginBottom: 8 }}>THEN (Action)</div>
              <select name="action_type" required style={{ width: '100%', padding: '8px 12px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', fontSize: 13, background: 'white', marginBottom: 8 }}>
                {actionTypes.map(a => <option key={a.value} value={a.value}>{a.label}</option>)}
              </select>
              <input name="action_value" placeholder="Value (rep name, stage, template, etc)" style={{ width: '100%', padding: '8px 12px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', fontSize: 13, boxSizing: 'border-box' as const }} />
            </div>
          </div>
      ...
      <div style={{ display: 'grid', gap: 10 }}>
        {workflows.map(wf => (
          <div key={wf.id} style={{ background: 'white', borderRadius: 16, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', padding: '20px 24px', display: 'flex', alignItems: 'center', gap: 20 }}>
            <div style={{ width: 40, height: 40, borderRadius: 12, background: wf.status === 'active' ? '#E1F5EE' : '#F5F5F5', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: '#1C1C1E' }}>{wf.name}</div>
            ...
            <div style={{ fontSize: 12, color: '#8E8E93', textAlign: 'right' }}>
              <div>{wf.executions} runs</div>
            </div>
            <button onClick={() => toggleStatus(wf)} style={{ padding: '6px 12px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', background: 'white', fontSize: 12, cursor: 'pointer', color: wf.status === 'active' ? '#854F0B' : '#085041' }}>
