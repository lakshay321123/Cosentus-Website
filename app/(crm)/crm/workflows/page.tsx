...
export default function WorkflowsPage() {
  ...
  if (loading) return <div style={{ padding: 40, color: '#9ca3af' }}>Loading workflows...</div>

  return (
    <div style={{ padding: '36px 44px', maxWidth: 1200 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 32, fontWeight: 300, color: '#1f2937', margin: 0 }}>Workflows</h1>
          <p style={{ fontSize: 14, color: '#9ca3af', margin: '4px 0 0' }}>Automate actions based on triggers</p>
        </div>
      ...
      {showCreate && (
        <form onSubmit={handleCreate} style={{ background: 'white', borderRadius: 16, border: '1px solid rgba(0,181,214,0.3)', padding: 24, marginBottom: 20 }}>
          ...
          <input name="name" placeholder="Workflow name *" required style={{ width: '100%', padding: '10px 14px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px #e5e7eb, 0 4px 12px #f3f4f6', fontSize: 13, marginBottom: 12, boxSizing: 'border-box' as const }} />
          <input name="description" placeholder="Description" style={{ width: '100%', padding: '10px 14px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px #e5e7eb, 0 4px 12px #f3f4f6', fontSize: 13, marginBottom: 16, boxSizing: 'border-box' as const }} />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div style={{ background: '#fafbfc', borderRadius: 12, padding: 16 }}>
              ...
              <select name="trigger_type" required style={{ width: '100%', padding: '8px 12px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px #e5e7eb, 0 4px 12px #f3f4f6', fontSize: 13, background: 'white', marginBottom: 8 }}>
                {Object.entries(triggerLabels).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
              </select>
              <div style={{ fontSize: 11, color: '#9ca3af', marginBottom: 8 }}>Condition (optional):</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 1fr', gap: 6 }}>
                <select name="condition_field" style={{ padding: '6px 8px', borderRadius: 4, border: 'none', boxShadow: '0 1px 3px #e5e7eb, 0 4px 12px #f3f4f6', fontSize: 12, background: 'white' }}>
                ...
                <select name="condition_op" style={{ padding: '6px 8px', borderRadius: 4, border: 'none', boxShadow: '0 1px 3px #e5e7eb, 0 4px 12px #f3f4f6', fontSize: 12, background: 'white' }}>
                ...
                <input name="condition_value" placeholder="Value" style={{ padding: '6px 8px', borderRadius: 4, border: 'none', boxShadow: '0 1px 3px #e5e7eb, 0 4px 12px #f3f4f6', fontSize: 12 }} />
              </div>
            </div>
            <div style={{ background: '#fafbfc', borderRadius: 12, padding: 16 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: '#00B5D6', marginBottom: 8 }}>THEN (Action)</div>
              <select name="action_type" required style={{ width: '100%', padding: '8px 12px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px #e5e7eb, 0 4px 12px #f3f4f6', fontSize: 13, background: 'white', marginBottom: 8 }}>
                {actionTypes.map(a => <option key={a.value} value={a.value}>{a.label}</option>)}
              </select>
              <input name="action_value" placeholder="Value (rep name, stage, template, etc)" style={{ width: '100%', padding: '8px 12px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px #e5e7eb, 0 4px 12px #f3f4f6', fontSize: 13, boxSizing: 'border-box' as const }} />
            </div>
          </div>
      ...
      <div style={{ display: 'grid', gap: 10 }}>
        {workflows.map(wf => (
          <div key={wf.id} style={{ background: 'white', borderRadius: 16, border: 'none', boxShadow: '0 1px 3px #e5e7eb, 0 4px 12px #f3f4f6', padding: '20px 24px', display: 'flex', alignItems: 'center', gap: 20 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: '#1f2937' }}>{wf.name}</div>
            ...
            <button onClick={() => toggleStatus(wf)} style={{ padding: '6px 12px', borderRadius: 10, border: 'none', boxShadow: '0 1px 3px #e5e7eb, 0 4px 12px #f3f4f6', background: 'white', fontSize: 12, cursor: 'pointer', color: wf.status === 'active' ? '#854F0B' : '#085041' }}>
