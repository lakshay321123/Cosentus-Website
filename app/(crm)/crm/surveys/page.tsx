...
export default function SurveysPage() {
  ...
  if (loading) return <div style={{ padding: 40, color: '#9ca3af' }}>Loading surveys...</div>

  return (
    <div style={{ padding: '36px 44px', maxWidth: 1200 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 32, fontWeight: 300, color: '#1f2937', margin: 0 }}>Surveys</h1>
          <p style={{ fontSize: 14, color: '#9ca3af', margin: '4px 0 0' }}>Client feedback — NPS, CSAT, custom surveys</p>
        </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 16, fontWeight: 600, color: '#1f2937' }}>{s.name}</span>
                    ...
                    <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 4, background: '#f3f4f6', color: '#9ca3af', textTransform: 'uppercase' }}>{s.type}</span>
                  </div>
                  ...
                  {resps.length === 0 ? (
                    ...
                    <div key={r.id} style={{ display: 'flex', gap: 12, padding: '8px 0', borderBottom: '0.5px solid #e5e7eb', fontSize: 13 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        {r.feedback && <div style={{ color: '#9ca3af', marginTop: 2 }}>{r.feedback}</div>}
                      </div>
                      <div style={{ fontSize: 11, color: '#d1d5db' }}>{new Date(r.created_at).toLocaleDateString()}</div>
                    </div>
                  ))}
                </div>
              )}
