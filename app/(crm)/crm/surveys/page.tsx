...
export default function SurveysPage() {
  ...
  if (loading) return <div style={{ padding: 40, color: '#616161' }}>Loading surveys...</div>

  return (
    <div style={{ padding: '32px 40px', maxWidth: 1200 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 300, color: '#000', margin: 0 }}>Surveys</h1>
          <p style={{ fontSize: 14, color: '#616161', margin: '4px 0 0' }}>Client feedback — NPS, CSAT, custom surveys</p>
        </div>
        <button onClick={() => setShowCreate(!showCreate)} style={{ background: '#00B5D6', color: 'white', border: 'none', borderRadius: 8, padding: '10px 24px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>+ Create Survey</button>
      </div>
      ...
      {showCreate && (
        <form onSubmit={handleCreate} style={{ background: 'white', borderRadius: 12, border: '1px solid #00B5D6', padding: 24, marginBottom: 20 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <input name="name" placeholder="Survey name *" required style={{ padding: '10px 14px', borderRadius: 6, border: '1px solid #E6E6E6', fontSize: 13 }} />
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
            <button type="submit" style={{ background: '#00B5D6', color: 'white', border: 'none', borderRadius: 6, padding: '8px 20px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Create Survey</button>
      ...
      <div style={{ display: 'grid', gap: 12 }}>
        {surveys.map(s => {
          ...
          return (
            <div key={s.id} style={{ background: 'white', borderRadius: 12, border: '1px solid #E6E6E6', padding: '20px 24px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 16, fontWeight: 600, color: '#000' }}>{s.name}</span>
                    ...
                    <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 4, background: '#F5F5F5', color: '#616161', textTransform: 'uppercase' }}>{s.type}</span>
                  </div>
                  <div style={{ fontSize: 12, color: '#616161', marginTop: 4 }}>{resps.length} responses · {s.questions.length} questions</div>
                </div>
                <div style={{ display: 'flex', gap: 6 }}>
                  <button onClick={() => toggleStatus(s)} style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid #E6E6E6', background: 'white', fontSize: 12, cursor: 'pointer' }}>{s.status === 'active' ? 'Close' : 'Activate'}</button>
                  <button onClick={() => viewResponses(s.id)} style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid #00B5D6', background: 'white', fontSize: 12, cursor: 'pointer', color: '#00B5D6' }}>{viewing === s.id ? 'Hide' : 'Responses'}</button>
                </div>
              </div>
              ...
              {viewing === s.id && (
                <div style={{ borderTop: '1px solid #F5F5F5', paddingTop: 12 }}>
                  {resps.length === 0 ? (
                    <div style={{ fontSize: 13, color: '#CCCCCC', textAlign: 'center', padding: 16 }}>No responses yet</div>
                  ) : resps.map(r => (
                    <div key={r.id} style={{ display: 'flex', gap: 12, padding: '8px 0', borderBottom: '1px solid #F5F5F5', fontSize: 13 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 500, color: '#000' }}>{r.respondent_name || r.respondent_email}</div>
                        {r.feedback && <div style={{ color: '#616161', marginTop: 2 }}>{r.feedback}</div>}
                      </div>
                      <div style={{ fontSize: 11, color: '#CCCCCC' }}>{new Date(r.created_at).toLocaleDateString()}</div>
                    </div>
                  ))}
                </div>
              )}
