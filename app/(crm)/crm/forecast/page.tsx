...
export default function ForecastPage() {
  ...
  if (loading) return <div style={{ padding: 40, color: '#8E8E93' }}>Loading forecast...</div>

  return (
    <div style={{ padding: '36px 44px', maxWidth: 1400 }}>
      <div style={{ marginBottom: 32 }}>
        <h1 style={{ fontSize: 32, fontWeight: 300, color: '#1C1C1E', margin: 0 }}>Revenue Forecast</h1>
        <p style={{ fontSize: 14, color: '#8E8E93', margin: '4px 0 0' }}>AI-weighted pipeline predictions based on stage probability</p>
      </div>
      ...
      <div style={{ background: 'white', borderRadius: 16, border: 'none', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)', padding: 24, marginBottom: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
          {[
            ...
            <div key={i} style={{ textAlign: 'center', padding: '24px 16px', borderRadius: 12, background: 'rgba(0,0,0,0.02)' }}>
              <div style={{ fontSize: 32, fontWeight: 300, color: '#00B5D6', lineHeight: 1 }}>${Math.round(m.value / 1000)}K</div>
              <div style={{ fontSize: 13, color: '#8E8E93', marginTop: 8 }}>{m.label}</div>
              ...
              <div style={{ fontSize: 11, color: '#C7C7CC', marginTop: 6 }}>{m.pct}% of pipeline</div>
            </div>
          ))}
        </div>
      </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {stageData.map(s => (
              <div key={s.stage} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 90, fontSize: 12, color: '#8E8E93', textTransform: 'capitalize', textAlign: 'right' }}>{s.stage}</div>
                <div style={{ flex: 1, height: 24, background: 'rgba(0,0,0,0.03)', borderRadius: 4, overflow: 'hidden', position: 'relative' }}>
                ...
                <div style={{ minWidth: 60, textAlign: 'right', fontSize: 12 }}>
                  ...
                  <span style={{ color: '#C7C7CC' }}> / ${Math.round(s.total / 1000)}K</span>
                </div>
                <div style={{ width: 32, fontSize: 11, color: '#8E8E93', textAlign: 'right' }}>{Math.round(s.pct * 100)}%</div>
              </div>
            ))}
          </div>
        </div>
            ...
            <thead>
              <tr style={{ borderBottom: '0.5px solid rgba(0,0,0,0.06)' }}>
                {['Rep', 'Deals', 'Raw', 'Weighted'].map(h => (
                  <th key={h} style={{ textAlign: h === 'Rep' ? 'left' : 'right', padding: '8px 0', fontWeight: 500, color: '#8E8E93', fontSize: 11, textTransform: 'uppercase' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {Object.entries(repMap).sort((a, b) => b[1].weighted - a[1].weighted).map(([rep, data]) => (
                <tr key={rep} style={{ borderBottom: '0.5px solid rgba(0,0,0,0.04)' }}>
                  ...
                  <td style={{ padding: '10px 0', textAlign: 'right', color: '#8E8E93' }}>${Math.round(data.total / 1000)}K</td>
