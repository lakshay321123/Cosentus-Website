  ...
  if (submitted) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Reddit Sans', sans-serif", background: '#fff' }}>
      <div style={{ textAlign: 'center', maxWidth: 400 }}>
        ...
        <p style={{ fontSize: 15, color: '#000' }}>Your feedback helps us improve our service.</p>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Reddit Sans', sans-serif", background: '#fff', padding: 20 }}>
      <div style={{ maxWidth: 520, width: '100%' }}>
        ...
        <p style={{ fontSize: 15, color: '#000', margin: '0 0 32px' }}>
          ...
          {Array.from({ length: maxScore + 1 }, (_, i) => isNPS ? i : i + 1).filter(i => i <= maxScore).map(n => {
            ...
            return (
              <button key={val} onClick={() => setScore(val)} style={{
                ...
                fontSize: 15, fontWeight: 600, cursor: 'pointer', fontFamily: "'Reddit Sans', sans-serif",
              }}>{val}</button>
            )
          })}
        </div>
        {isNPS && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#CCCCCC', marginTop: -20, marginBottom: 24 }}><span>Not likely</span><span>Very likely</span></div>}

        <textarea value={feedback} onChange={e => setFeedback(e.target.value)} placeholder="Any additional feedback? (optional)"
          rows={3} style={{ width: '100%', padding: '12px 14px', borderRadius: 10, border: '1px solid #E6E6E6', fontSize: 14, fontFamily: "'Reddit Sans', sans-serif", marginBottom: 16, boxSizing: 'border-box', resize: 'vertical' }} />

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 20 }}>
          <input value={name} onChange={e => setName(e.target.value)} placeholder="Your name (optional)" style={{ padding: '10px 14px', borderRadius: 10, border: '1px solid #E6E6E6', fontSize: 14, fontFamily: "'Reddit Sans', sans-serif" }} />
          <input value={email} onChange={e => setEmail(e.target.value)} placeholder="Your email (optional)" style={{ padding: '10px 14px', borderRadius: 10, border: '1px solid #E6E6E6', fontSize: 14, fontFamily: "'Reddit Sans', sans-serif" }} />
        </div>
        ...
        <button onClick={handleSubmit} disabled={score === null} style={{
          ...
          color: score !== null ? '#fff' : '#CCCCCC', border: 'none', fontSize: 15, fontWeight: 600,
