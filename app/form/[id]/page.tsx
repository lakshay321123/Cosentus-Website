  ...
  if (submitted) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Reddit Sans', sans-serif" }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 'var(--text-base)', color: '#000' }}>We will be in touch shortly.</p>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Reddit Sans', sans-serif", padding: 20 }}>
      <div style={{ maxWidth: 480, width: '100%' }}>
        ...
        {form.description && <p style={{ fontSize: 'var(--text-sm)', color: '#000', margin: '0 0 24px' }}>{form.description}</p>}
        <form onSubmit={handleSubmit}>
          {form.fields.map((f: any, i: number) => (
            <div key={i} style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 'var(--text-xs)', fontWeight: 600, color: '#000', display: 'block', marginBottom: 4 }}>{f.label}{f.required && ' *'}</label>
              {f.type === 'textarea' ? (
                <textarea value={values[f.label] || ''} onChange={e => setValues({ ...values, [f.label]: e.target.value })} required={f.required} rows={3}
                  style={{ width: '100%', padding: '10px 14px', borderRadius: 10, border: '1px solid #E6E6E6', fontSize: 'var(--text-sm)', fontFamily: "'Reddit Sans', sans-serif", boxSizing: 'border-box', resize: 'vertical' }} />
              ) : f.type === 'select' ? (
                <select value={values[f.label] || ''} onChange={e => setValues({ ...values, [f.label]: e.target.value })} required={f.required}
                  style={{ width: '100%', padding: '10px 14px', borderRadius: 10, border: '1px solid #E6E6E6', fontSize: 'var(--text-sm)', background: '#fff' }}>
                ...
                <input type={f.type === 'phone' ? 'tel' : f.type} value={values[f.label] || ''} onChange={e => setValues({ ...values, [f.label]: e.target.value })} required={f.required}
                  style={{ width: '100%', padding: '10px 14px', borderRadius: 10, border: '1px solid #E6E6E6', fontSize: 'var(--text-sm)', fontFamily: "'Reddit Sans', sans-serif", boxSizing: 'border-box' }} />
              )}
            </div>
          ))}
          <button type="submit" disabled={submitting} style={{ width: '100%', padding: 14, borderRadius: 10, background: '#00B5D6', color: '#fff', border: 'none', fontSize: 'var(--text-base)', fontWeight: 600, cursor: submitting ? 'wait' : 'pointer', opacity: submitting ? 0.6 : 1, fontFamily: "'Reddit Sans', sans-serif" }}>
            {submitting ? 'Submitting...' : 'Submit'}
          </button>
        </form>
        <div style={{ textAlign: 'center', marginTop: 28, fontSize: 'var(--text-xxs)', color: '#CCCCCC' }}>Cosentus · cosentus.com</div>
      </div>
    </div>
  )
}
