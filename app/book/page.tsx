    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f8f9fb', fontFamily: "'Reddit Sans', sans-serif" }}>
        <div style={{ maxWidth: 500, textAlign: 'center', padding: 40 }}>
          ...
          <p style={{ fontSize: 'var(--text-base)', color: '#000000', margin: '0 0 8px' }}>
          ...
          <p style={{ fontSize: 'var(--text-sm)', color: '#000000', marginBottom: 32 }}>Check your email for a calendar invite and confirmation.</p>
          <Link href="/" style={{ color: '#00B5D6', textDecoration: 'none', fontSize: 'var(--text-sm)', fontWeight: 500 }}>← Back to Cosentus.com</Link>
        </div>
      </div>
    )
  }
    ...
    <div style={{ minHeight: '100vh', background: '#f8f9fb', fontFamily: "'Reddit Sans', sans-serif" }}>
      <div style={{ maxWidth: 700, margin: '0 auto', padding: '60px 20px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          ...
          <p style={{ fontSize: 'var(--text-base)', color: '#000000', margin: 0 }}>Pick a time that works. No pressure. Just data.</p>
        </div>
            ...
            {meetingTypes.map(t => (
              ...
              onMouseLeave={e => { e.currentTarget.style.borderColor = '#E6E6E6' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <div style={{ fontSize: 'var(--text-base)', fontWeight: 600, color: '#000' }}>{t.label}</div>
                    <div style={{ fontSize: 'var(--text-sm)', color: '#000000', marginTop: 4 }}>{t.desc}</div>
                  </div>
                  <span style={{ fontSize: 'var(--text-xs)', color: '#00B5D6', fontWeight: 500, whiteSpace: 'nowrap' }}>{t.duration}</span>
                </div>
              </button>
            ))}
          </div>
        ...
        {step === 2 && (
          <div style={{ background: 'white', borderRadius: 12, border: '1px solid #E6E6E6', padding: 24 }}>
            <h3 style={{ fontSize: 'var(--text-base)', fontWeight: 600, margin: '0 0 16px' }}>Pick a Date</h3>
              ...
              {dates.map(d => {
                ...
                return (
                  <button key={d.toISOString()} onClick={() => setSelectedDate(d)} style={{
                    ...
                    fontSize: 'var(--text-xs)', fontWeight: isSelected ? 600 : 400, minWidth: 80, textAlign: 'center',
                  }}>
            ...
            {selectedDate && (
              <>
                <h3 style={{ fontSize: 'var(--text-base)', fontWeight: 600, margin: '0 0 16px' }}>Pick a Time</h3>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
                  {timeSlots.map(t => (
                    <button key={t} onClick={() => { setSelectedTime(t); setStep(3) }} style={{
                      ...
                      color: selectedTime === t ? 'white' : '#000', fontSize: 'var(--text-xs)',
                    }}>{t}</button>
                  ))}
                </div>
              </>
            ...
            <button onClick={() => setStep(1)} style={{ marginTop: 16, background: 'none', border: 'none', color: '#000000', fontSize: 'var(--text-xs)', cursor: 'pointer' }}>← Back</button>
          </div>
        )}
        ...
        {step === 3 && (
          <div style={{ background: 'white', borderRadius: 12, border: '1px solid #E6E6E6', padding: 24 }}>
            <h3 style={{ fontSize: 'var(--text-base)', fontWeight: 600, margin: '0 0 4px' }}>Almost Done</h3>
            <p style={{ fontSize: 'var(--text-xs)', color: '#000000', marginBottom: 20 }}>
              {meetingTypes.find(m => m.id === meetingType)?.label} · {selectedDate?.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })} at {selectedTime}
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <input value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} placeholder="Your name *" required
                style={{ padding: '12px 14px', borderRadius: 6, border: '1px solid #E6E6E6', fontSize: 'var(--text-sm)' }} />
              ...
              <select value={formData.specialty} onChange={e => setFormData({ ...formData, specialty: e.target.value })}
                style={{ padding: '12px 14px', borderRadius: 6, border: '1px solid #E6E6E6', fontSize: 'var(--text-sm)', background: 'white', color: formData.specialty ? '#000' : '#616161' }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 20 }}>
              <button onClick={handleBook} disabled={!formData.name || !formData.email} style={{
                ...
                padding: '14px 32px', fontSize: 'var(--text-sm)', fontWeight: 600, cursor: 'pointer',
                opacity: (!formData.name || !formData.email) ? 0.5 : 1,
              }}>Book Meeting</button>
              <button onClick={() => setStep(2)} style={{ background: 'none', border: '1px solid #E6E6E6', borderRadius: 8, padding: '14px 20px', fontSize: 'var(--text-xs)', cursor: 'pointer', color: '#000000' }}>← Back</button>
            </div>
          </div>
        )}

        <div style={{ textAlign: 'center', marginTop: 40, fontSize: 'var(--text-xxs)', color: '#000000' }}>
