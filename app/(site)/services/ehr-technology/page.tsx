...
export default async function EHRTechnologyPage() {
  
  let page: any = null
  try { page = await getPageData('ehr-technology') } catch (e) {}

  return (
    <main>
      <PageHero
        label="EHR & TECHNOLOGY"
        title={page?.heroHeadline || "EHR Agnostic. Seamlessly Integrated."}
        subtitle={page?.heroSubtitle || "Works with your existing EHR — Epic, Athenahealth, eClinicalWorks, AdvancedMD, ModMed, nxGen, and more. No migrations. No disruption. For practices that want a purpose-built option, Medcloud supports cleaner claims and faster revenue."}
